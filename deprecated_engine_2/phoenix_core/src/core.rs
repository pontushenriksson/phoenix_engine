...
use crate::ecs::components::{Texture2D, TextureType};
...
impl PhoenixEngine {
  ...
  pub fn run<F: FnMut()>(&mut self, mut logic: F) /* -> Result<PhoenixLogPath, Vec<ErrorMessage>> */ {  
    ...
    while !self.window.should_close() {
      ...
      // Render all objects in the static_game_objects vector
      for entry in &self.static_objects {
        entry.render(&mut self.cameras_3d, &self.textures, &mut self.shaders, &mut self.window);
      }
...
impl PhoenixEngine {
  pub fn create_texture2D(path: &str, texture_type: TextureType) -> Resource {
    Resource::Texture(Texture2D::new(path, texture_type))
  }
  ...
  pub fn new_static_object(
    ...
    let mut shader = self.shaders.get_mut(shader_program).unwrap();
    shader.bind();
    shader.create_uniform("model");
    shader.set_uniform_matrix_4_f32_vec("model", &self.static_objects.last().unwrap().model_matrix);
  }
  ...
  pub fn world_space_vector_3d(x: f32, y: f32, z: f32) -> cgmath::Vector3<f32> {
    cgmath::vec3(x, y, z)
  }

  pub fn id_matrix() -> cgmath::Matrix4<f32> {
